...
const help = `LoopMe mechanical helpers (Node.js >=22)
  ...
  node loopme.mjs check-config [--config FILE] [--herdr EXECUTABLE]
  node loopme.mjs init [--config FILE] [--herdr EXECUTABLE] [--root DIRECTORY]
...
init requires HERDR_ENV=1 and creates a fresh task draft, not an executable contract.
...
async function main() {
  ...
  const definitions = {
    'check-config': { config: { type: 'string' }, herdr: { type: 'string' } },
    init: { config: { type: 'string' }, herdr: { type: 'string' }, root: { type: 'string' } },
  ...
  if (action === 'init' || action === 'check-config') {
    ...
    result = action === 'init'
      ? await initialize(values.config, { herdr: values.herdr, root: values.root })
      : await checkConfig(values.config, { herdr: values.herdr });
  } else {
