...
Outer owns acceptance semantics, minimum proof requirements and the final PASS/REJECT.
Goal owns methods and evidence and may implement or refactor fixtures and assertion
code without weakening expectations, rejection conditions or the required proof
boundary. Changing acceptance meaning is an Outer-owned contract change.
...
| Ownership and Stop Protocol | Which decisions belong to Outer (acceptance semantics, minimum proof, PASS/REJECT) and which belong to Goal (methods, fixtures, assertion code), plus the handoff markers and the evidence-row update duty. | Name the boundary, not the implementation: Goal may refactor tests but never weaken expectations, rejection conditions or the required proof boundary. |
...
### Proof boundary and test seams

The proof boundary and the test seam are different decisions. Outer fixes the boundary:
which entry, artifact or build must carry the proof. Goal declares the seam before
implementing the slice — where input is applied, where the result is observed, which
dependencies are real and which are replaced, and what this check cannot yet prove.
Refining the seam is a method change and needs no extra approval while the frozen
boundary, meaning and minimum proof stay intact; a change that would move the boundary
is a contract change and returns to Outer.

## Sequence: red first, one slice at a time

For new or changed objective behavior with a reachable execution boundary, run the
smallest acceptance check for the current slice before implementing it, and confirm it
fails because the target behavior is not yet met. A red result caused by a missing tool,
misconfiguration, unrelated exception or compile error is not that evidence: fix the
setup, or record the proof gap. Where the boundary is unreachable, keep the gap visible
instead of manufacturing a failure record. Preserve an existing green baseline for pure
refactors, and verify document or low-risk static artifacts by their stated rejection
rule rather than by a constructed bad artifact.

Prefer one minimal end-to-end slice over layer-by-layer batches when a change crosses
modules, processes or integration points: drive one key behavior through the agreed
entry to an observable result, verify that slice, then extend. A mechanism experiment
and a real-entry proof are not interchangeable — an experiment can establish a local
property while the assembled call chain is still wrong, so neither may stand in for the
other.

Under concurrent writers, keep the batch small enough that a slice, or one group of
clearly dependent edits, settles and is verified before the next batch starts. Deferring
tests during concurrent writes shortens the safe window; it does not license verifying
everything after the bulk of the work is done.
